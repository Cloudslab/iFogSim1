...
public class LargeSimulationMultipleQueries {
			...
			for(int i=0;i<8;i++){
				for(int j=0;j<8;j++){
					...
					for(int k=0;k<numQueries;k++){
						createSensor(k, "sensor-TYPE"+k+"-"+i+"-"+j+"-0", queries.get(k), broker.getId(), CloudSim.getEntityId("gateway-"+id), transmitInterval, sensorTupleCpuSize, sensorTupleNwSize);
						createSensor(k, "sensor-TYPE"+k+"-"+i+"-"+j+"-1", queries.get(k), broker.getId(), CloudSim.getEntityId("gateway-"+id), transmitInterval, sensorTupleCpuSize, sensorTupleNwSize);
						createSensor(k, "sensor-TYPE"+k+"-"+i+"-"+j+"-2", queries.get(k), broker.getId(), CloudSim.getEntityId("gateway-"+id), transmitInterval, sensorTupleCpuSize, sensorTupleNwSize);
						createSensor(k, "sensor-TYPE"+k+"-"+i+"-"+j+"-3", queries.get(k), broker.getId(), CloudSim.getEntityId("gateway-"+id), transmitInterval, sensorTupleCpuSize, sensorTupleNwSize);
					}
				}
			}
	...
	private static List<FogDevice> createFogDevices(int userId) {
		...
		level2Devices.add(createFogDevice("level2-0", 1000, new GeoCoverage(0, 40, 0, 40), 10000, 1));
		level2Devices.add(createFogDevice("level2-1", 1000, new GeoCoverage(0, 40, 40, 80), 10000, 1));
		level2Devices.add(createFogDevice("level2-2", 1000, new GeoCoverage(40, 80, 0, 40), 10000, 1));
		level2Devices.add(createFogDevice("level2-3", 1000, new GeoCoverage(40, 80, 40, 80), 10000, 1));
	...
	private static StreamQuery createStreamQuery(final int queryIndex, int userId, int transmitInterval){
		...
		final StreamOperator spout = new StreamOperator(FogUtils.generateEntityId(), spoutName, null, "TYPE"+queryIndex, queryId, userId, 
				mips, ram, bw, size, vmm, new TupleScheduler(mips, 1), 1, 1, 10, 200, 256/((double)transmitInterval));
		final StreamOperator bolt = new StreamOperator(FogUtils.generateEntityId(), boltName, null, null, queryId, userId, 
				mips, ram, bw, size, vmm, new TupleScheduler(mips, 1), 1, 1, 20, 100, 256/((double)transmitInterval));
		List<StreamOperator> operators = new ArrayList<StreamOperator>(){{add(spout);add(bolt);}};
		Map<String, String> edges = new HashMap<String, String>(){{put(spoutName, boltName);}};
		...
		add(new OperatorEdge(spoutName, boltName, 0.1));}};
