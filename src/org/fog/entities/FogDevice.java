...
public class FogDevice extends Datacenter {
	...
	public FogDevice(
			...
			double uplinkBandwidth, double latency, double downlinkBandwidth) throws Exception {
			...
			for (Vm vm : host.getVmList()) {
				while (vm.getCloudletScheduler().isFinishedCloudlets()) {
					...
					if (cl != null) {
						...
						//Logger.debug(getName(), "Tuples generated by module "+tuple.getDestModuleName());
	...
	private void sendTupleToActuator(Tuple tuple){
		int actuatorId = tuple.getActuatorId();
	...
	private void processTupleArrival(SimEvent ev){
		...
		if(appToModulesMap.containsKey(tuple.getAppId())){
			if(appToModulesMap.get(tuple.getAppId()).contains(tuple.getDestModuleName())){
				...
				Logger.debug(getName(), "Received tuple "+tuple.getTupleType()+" for module "+tuple.getDestModuleName());
